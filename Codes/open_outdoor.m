% 'open_outdoor_trajectory' MATLAB file plots the trajectory of RTK-GPS and  VINS in Google Map for visualization purposes.
% 
...
% 'open_outdoor_vins'  bag contains data for VINS trajectory and
% 'open_outdoor'  bag contains data for RTK GPS trajectory
bag1 = rosbag ('C:\Users\aruns\OneDrive\Desktop\RSN\final_project\open_outdoor_vins.bag');
bsel1 = select(bag1,'Topic','/FrameTrajectory_TUM_Format');
bag2 = rosbag ('C:\Users\aruns\OneDrive\Desktop\RSN\final_project\open_outdoor.bag');
...
lat = lat_a(1:length(lat_a));
lon = lon_a(1:length(lon_a));
...
gps_timePoints = gps_timePoints_a(1:length(gps_timePoints_a));
...
figure;
% plot((R_x_e(:,1) - R_x_e(1,1))+15.4562, (R_x_e(:,2) - R_x_e(1,2))+12.55,'DisplayName','Aligned-VINS-trajectory');
plot((R_x_e(:,1) - R_x_e(1,1))+1.3, (R_x_e(:,2) - R_x_e(1,2))-1,'DisplayName','Aligned-VINS-trajectory');
hold on;
...
vins_delta_x = ((R_x_e(:,1) - R_x_e(1,1))) + 1.3;
vins_delta_y = ((R_x_e(:,2) - R_x_e(1,2))) + 1;
...
figure;
plot(lon,lat,'Color','b','LineWidth',4);
hold on;
plot(Lon,Lat,'Color','r','LineWidth',4);
